...
function resortSummaryEvents(events: AgentEvent[]) {
	...
	const lastInvolvedSummaryCompressionEventIndex = events.findLastIndex(event =>
		event.meta?.roundId === lastInvolvedRoundId
		&& event.meta?.turn === lastInvolvedTurn
	)
	...
	return [
		lastSummaryEvent,
		...availableEvents.filter(event => event.type !== EventType.AGENT_SUMMARY)
	]
