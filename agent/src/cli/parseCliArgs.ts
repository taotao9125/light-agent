...
export function parseCliArgs(argv: string[] = process.argv.slice(2), cwd = process.cwd()): CliArgs {
	...
	const sessionFile = resolvePath(cwd, values['session-file'] ?? DEFAULT_SESSION);
	const traceFile = resolvePath(cwd, values['trace-file'] ?? defaultSidecarFile(sessionFile, 'trace'));
	const contextFile = resolvePath(cwd, values['context-file'] ?? defaultSidecarFile(sessionFile, 'context'));
