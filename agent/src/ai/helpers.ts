...
export function toRoundEventMap(events: AgentEvent[]): Map<string, AgentEvent[]> {
	const roundEventMap = new Map<string, AgentEvent[]>();
	for (const event of events) {
		const roundId = event.meta?.roundId;
		if (!roundId) continue;

		if (!roundEventMap.has(roundId)) {
			roundEventMap.set(roundId, []);
		}
		roundEventMap.get(roundId)!.push(event);
	}

	return roundEventMap;
}
...
export function splitEventsToRoundGroups(events: AgentEvent[]): EventRoundGroup[] {
	return [...toRoundEventMap(events).values()].map((roundEvents) => ({
