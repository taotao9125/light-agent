...
function getRequiredEnv(name: string): string {
  ...
  if (!value) {
    console.error(`请在 mcp_server/.env 中设置 ${name}`);
    process.exit(1);
  }
  return value;
}
...
type ContextBuilder = {
  addContext(message?: ChatCompletionMessageParam, tool?: ChatCompletionTool): AgentContext;
...
async function createBooking({ room_id, start_time, end_time }: CreateBookingArgs) {
  return axios.request({
    method: 'post',
...
const toolDefinitions: ToolDefinition[] = [
  ...
  },
  {
    ...
    handler: (args) => createBooking(args as CreateBookingArgs),
...
const LLMTools: ChatCompletionTool[] = toolDefinitions.map(tool => ({
  type: 'function',
  function: {
    name: tool.name,
...
async function runToolFromCall(toolCall: ChatCompletionMessageFunctionToolCall): Promise<unknown> {
...
async function LLMReasoning(client: OpenAI, context: AgentContext): Promise<ChatCompletion> {
...
function buildContext(initMessages: ChatCompletionMessageParam[] = [], initTools: ChatCompletionTool[] = []): ContextBuilder {
  const initContext: AgentContext = {
    messages: initMessages,
    tools: initTools
  }

  return {
    addContext: (message?: ChatCompletionMessageParam, tool?: ChatCompletionTool) => {
      if (message) {
...
function emitAgentEvent(type: string, payload: AgentEventPayload = {}) {
  console.log(
    JSON.stringify({
      type,
...
function isFinalAnswer(message: ChatCompletionMessage): boolean {
  return !message.tool_calls?.length;
}

function getFunctionToolCalls(message: ChatCompletionMessage): ChatCompletionMessageFunctionToolCall[] {
  return (message.tool_calls ?? []).filter(
    (toolCall): toolCall is ChatCompletionMessageFunctionToolCall => toolCall.type === 'function'
  );
}

async function executeToolCalls(toolCalls: ChatCompletionMessageFunctionToolCall[], contextBuilder: ContextBuilder) {
...
function createLoopGuard({ maxReasoningTurns, maxToolCalls, maxDurationMs }: LoopGuardOptions) {
...
async function runToolUseFlow(client: OpenAI, contextBuilder: ContextBuilder): Promise<{ text: string }> {
  ...
  while (true) {
    ...
    // 构建下一轮 context
    contextBuilder.addContext(nextPlan as ChatCompletionAssistantMessageParam);

    const toolCalls = getFunctionToolCalls(nextPlan);
    await executeToolCalls(toolCalls, contextBuilder);

    loopGuard.recordReasoningTurns();
    loopGuard.recordToolCalls(toolCalls.length);
  }
